...
# Ralph 10 Iterations Script
# Runs Ralph with fresh context each iteration
...
for i in {1..10}; do
    ...
    claude --dangerously-skip-permissions -p "You are Ralph. Iteration $i. Work on ONE story per iteration. Fresh context each iteration.
...
Now begin: Read PRD.md first to identify the next incomplete task, then implement it following all the steps above." || true

    # Wait 5 seconds between iterations to avoid rate limits
    if [ $i -lt 10 ]; then
