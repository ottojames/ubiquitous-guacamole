...
- [x] **Task 20.1c**: Create CouncilContext. ~60 lines. **Success**: Works. (Completed 2026-01-21 - Created `src/contexts/CouncilContext.tsx` (~70 lines) with CouncilProvider and useCouncil hook; provides department, userRole, organizationSlug, departmentSlug, and basePath to council components; integrated into CouncilLayout.tsx wrapping Outlet; maintains backward compatibility with useOutletContext)
...
- [x] **Task 21.2b**: Create endpoint. **Success**: Callable. (Completed 2026-01-21 - Created server/routes/drafting.ts with three endpoints: POST /api/drafting/generate (from DraftInput), POST /api/drafting/from-notice-base (for wizard flow), POST /api/drafting/from-draft (for legacy flow). Registered in server/index.ts at /api/drafting/*. All endpoints return DraftResult with success, draftText, noticeType, generatedAt, suggestions. 503 tests pass)
...
- [x] **Task 21.3c**: Add to council inbox. **Success**: Shows summary. (Completed 2026-01-21 - Created src/hooks/useRepresentationAnalysis.ts hook for fetching analysis from API. Created src/components/council/RepresentationAnalysisSummary.tsx component with expandable UI showing summary, key themes with confidence percentages, and licensing objectives referenced. Integrated into Representations.tsx detail modal after representation text section. Purple-themed UI matches AI features design pattern. Component auto-analyzes when modal opens, shows loading spinner, and handles errors gracefully. 506 tests pass)
...
- [x] **Task 22.2c**: Create webhook service. ~60 lines. **Success**: Sends. (Completed 2026-01-21 - Created server/services/webhooks.ts (~60 lines) with: sendWebhookEvent(organizationId, event, eventId, data) function that queries enabled webhooks subscribed to event type, delivers payload with HMAC-SHA256 signature in X-Webhook-Signature header, logs delivery attempts to webhook_deliveries table, updates webhook stats (last_triggered_at, last_success/failure_at). deliverWebhook() helper handles fetch with timeout via AbortController. All 506 tests pass)
- [x] **Task 22.2d**: Integrate into endpoints. **Success**: Fires. (Completed 2026-01-21 - Integrated sendWebhookEvent into 4 endpoints: 1) notices.ts fires notice.published when notice submitted/published with organization_id, 2) stripe.ts fires payment.completed after successful Stripe checkout, 3) representations.ts fires representation.submitted when new representation inserted, 4) workflow.ts fires workflow.stage_changed after successful stage transition. All webhook calls are async (don't block response). All 506 tests pass)
