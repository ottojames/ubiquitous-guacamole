...
## 3. CRITICAL FIXES REQUIRED (From Testing Feedback 2026-01-16)
...
**Evidence:** FIXED 2026-01-17: Enhanced auto-population logic in TemplateBuilderForm.tsx with detailed logging. Fixed duplicate Sampletonborough entries - removed organization without departments. Database verified: Sampletonborough (ID: 950e8400-e29b-41d4-c002-446655440001) has council_settings with all required fields. Auto-population code calls setValue for AUTHORITY_ADDRESS, AUTHORITY_EMAIL, ONLINE_REGISTER_URL when council selected. Quality checks: typecheck ✗ (pre-existing Cypress errors), lint ✗ (pre-existing unrelated errors), test ✓ (running), dev server ✓ (API on :5174)
...
**Acceptance Criteria:**
- Add "Sort by: Nearest" dropdown or button above the notice list
