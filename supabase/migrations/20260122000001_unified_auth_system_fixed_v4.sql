-- ============================================================================
-- UNIFIED AUTHENTICATION SYSTEM MIGRATION (FIXED V4)
-- Migrates from dual auth to single Supabase Auth
-- Fixed: Disables triggers during data cleanup to avoid validation errors
-- ============================================================================
...
BEGIN
  -- Check if validate_notice trigger exists
  SELECT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'validate_notice'
    AND tgrelid = 'public.notices'::regclass
  ) INTO trigger_exists;

  -- Disable the trigger temporarily if it exists
  IF trigger_exists THEN
    ALTER TABLE public.notices DISABLE TRIGGER validate_notice;
  END IF;

  -- Also check for validate_notice_organization trigger
  IF EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'validate_notice_organization'
    AND tgrelid = 'public.notices'::regclass
  ) THEN
    ALTER TABLE public.notices DISABLE TRIGGER validate_notice_organization;
  END IF;
  ...
  -- Clear department_id for notices that will be reassigned
  -- This prevents the trigger from failing when we update organization_id
  UPDATE public.notices
  SET department_id = NULL
  WHERE organization_id IS NULL
     OR NOT EXISTS (
       SELECT 1 FROM public.organizations o
       WHERE o.id = notices.organization_id
     );
  ...
  IF trigger_exists THEN
    ALTER TABLE public.notices ENABLE TRIGGER validate_notice;
  END IF;
  ...
  IF EXISTS (
    ...
    ALTER TABLE public.notices ENABLE TRIGGER validate_notice_organization;
  END IF;
