-- Migration: Add Authorization Checks to SECURITY DEFINER Functions
-- Description: SECURITY DEFINER functions bypass RLS, so we must manually
--              verify the caller has permission before executing.
--
-- Security Issue: Without these checks, any authenticated user could call
--                 these functions with any firm_id, bypassing tenant isolation.
--
-- Fix: Add EXISTS checks to verify auth.uid() is a member of the firm
--      before allowing the function to proceed.

-- ============================================================================
-- HELPER FUNCTION: Check firm membership
-- ============================================================================
-- Create a helper function to verify firm membership
-- This makes the auth checks more consistent and maintainable

CREATE OR REPLACE FUNCTION auth.is_firm_member(p_firm_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
...
COMMENT ON FUNCTION auth.is_firm_member IS 'Returns true if the current user is a member of the specified organization/firm';

-- ============================================================================
-- 1. transition_notice_stage - Add firm membership check
-- ============================================================================
...
BEGIN
  ...
  IF NOT auth.is_firm_member(v_workflow_status.firm_id) THEN
...
-- ============================================================================
-- 2. initialize_notice_workflow - Add firm membership check
-- ============================================================================
...
BEGIN
  -- AUTHORIZATION CHECK: Verify caller is member of the firm
  IF NOT auth.is_firm_member(p_firm_id) THEN
...
-- ============================================================================
-- 3. create_default_premises_licence_workflow - Add firm membership check
-- ============================================================================
...
-- ============================================================================
-- 4. create_default_probate_workflow - Add firm membership check
-- ============================================================================
...
-- ============================================================================
-- 5. create_default_planning_workflow - Add firm membership check
-- ============================================================================
...
-- ============================================================================
-- 6. create_default_tro_workflow - Add firm membership check
-- ============================================================================
...
-- ============================================================================
-- 7. create_default_gvol_workflow - Add firm membership check
-- ============================================================================
...
-- ============================================================================
-- 8. create_default_gambling_workflow - Add firm membership check
-- ============================================================================
...
-- SUMMARY OF CHANGES
-- ============================================================================
-- Added authorization checks to 8 SECURITY DEFINER functions:
--
-- 1. auth.is_firm_member(p_firm_id) - NEW helper function for membership checks
-- 2. transition_notice_stage - verifies user is member of the notice's firm
-- 3. initialize_notice_workflow - verifies user is member of the specified firm
-- 4. create_default_premises_licence_workflow - verifies user is firm member
-- 5. create_default_probate_workflow - verifies user is firm member
-- 6. create_default_planning_workflow - verifies user is firm member
-- 7. create_default_tro_workflow - verifies user is firm member
-- 8. create_default_gvol_workflow - verifies user is firm member
-- 9. create_default_gambling_workflow - verifies user is firm member
--
-- These checks prevent any authenticated user from calling these functions
-- with arbitrary firm_id values, which would bypass RLS tenant isolation.
