...
async function testRepresentations() {
  try {
    ...
    if (orgError) {
      console.error('Error fetching Westminster org:', orgError);
      return;
    }

    // Get Westminster Licensing department specifically
    ...
    if (!reps || reps.length === 0) {
      ...
      if (insertError) {
        ...
        console.log('Created 3 test representations successfully:', insertedReps.map(r => r.id));
      }

      // Test the query again
      const { data: newReps, error: newRepsError } = await supabase
        .from('representations')
        .select(`
          *,
          notices!inner (
      ...
      console.log('\nAfter creating test data - Representations:', newReps?.length || 0);
    }
